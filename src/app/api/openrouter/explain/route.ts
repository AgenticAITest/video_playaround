...
export async function POST(request: NextRequest) {
    try {
        ...
        const { workflowSummary, openRouterApiKey, model } = body as {
        ...
        if (!content) {
            return NextResponse.json({ explanation: null, error: "No response from OpenRouter" });
        }
        ...
        try {
            ...
            // If OpenRouter returns non-JSON, wrap it
            return NextResponse.json({
                explanation: {
                    summary: content,
                    nodeGroups: [],
